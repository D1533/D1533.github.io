---
layout: default
title: Very Smooth
---
---
## PicoCTF - Very Smooth
---

## Description
Forget safe primes... Here, we like to live life dangerously... >:)

---
## Solution

We say that a number $$n \in \mathbb{Z}$$ is $$B$$-smooth if all the primes factors of $$n$$ are less or equal than $$B$$. That is,
{% raw %}
$$n = p_1^{e_1} p_2^{e_2} \cdots p_k^{e_k} \implies p_j \leq B \ \ \forall j = 1, \cdots, k.$$
{% endraw %}

We will use a well-known factoring algorithm called Pollard's p-1 method.
Pollard's p-1 factoring method leverages the fact that if $$p$$ is a prime divisor of $$n$$ and $$p-1$$ is $$B$$-smooth, then for some suitably chosen $$M$$ such that $$M \mid (p-1)$$, i.e , $$M = k(p-1)$$ for some $$ k \in \mathbb{Z}$$, Fermat's Little Theorem implies
{% raw %}
$$a ^ M \equiv a^{k(p-1)} \equiv (a^{p-1})^k \equiv 1 \ mod \ p,$$
{% endraw %}
and therefore $$p \mid (a^M - 1)$$. 

Since $$p$$ also divides $$n$$, it follows that
{% raw %}
$$gcd(a^M-1,n)$$
{% endraw %}
might return a non-trivial factor of $$n$$.

There are efficient ways to find such $$M$$, but for this challenge, brute forcing it is sufficient.

Once $$p$$ is found, we can recover $$q = n/p$$, compute the private key $$d = e^{-1} \ mod \ \varphi(n)$$, where $$\varphi(n) = (p-1)(q-1)$$, and decrypt the flag as $$m = c ^ d \ mod \ n$$.

```python
from math import gcd
from Crypto.Util.number import long_to_bytes

n = 
c = 
e = 0x10001


def pollard(n):
    a = 2
    j = 2
    while True: 
        a = pow(a,j,n)
        g = gcd(a-1,n)
        if 1 < g and g < n:
            return g
        j += 1


p = pollard(n)
q = n//p
phi = (p-1)*(q-1)
d = pow(e,-1,phi)
flag = long_to_bytes(pow(c,d,n)).decode()
print(flag)
    

```